...
export const TaskCreatePanel: Component<{
  ...
  return (
    ...
    >
      ...
      >
        <div
          class="relative w-[320px] flex flex-col items-center gap-[16px] rounded-[8px] bg-white px-[16px] py-[20px] transition-transform duration-300 ease-out"
          classList={{ "scale-90": scale() }}
        ...
        >
          <img
            ...
            onClick={() => props.onClose()}
          />
          <div class="text-[1.2em] font-medium text-[#0f86ff]">创建下载任务</div>
          ...
          >
            ...
            <FormItem label="自动转码">
              <RadioButtonGroup name="autoTranscode">
                <RadioButton label="是（推荐）" value="on" checked />
                <RadioButton label="否（仅保存 TS）" value="off" />
              </RadioButtonGroup>
            </FormItem>

            <div class="mt-[4px] w-full rounded-[6px] bg-[#f7f9fc] p-[10px] text-[12px] leading-5 text-[#666]">
              流程：抓取<span class="font-medium">投影流（VGA）</span> → 分片上传 →
              服务器合并/（可选）转封装 → 完成后可直接下载 MP4。
              <br />
              已处理的视频会自动复用，避免重复上传与解析。
            </div>

            <button class="ant-btn ant-btn-primary mt-[12px]" type="submit">
              创建
            </button>
          </form>
        </div>
...
const FormItem: ParentComponent<{ label: string }> = (props) => {
  return (
    <div class="mb-[14px] w-full flex flex-row items-center gap-[12px]">
      <label class="flex-shrink-0 flex-basis-[78px] text-right text-[#666]">
        {props.label}
...
const RadioButtonGroup: ParentComponent<{ name: string }> = (props) => {
  ...
  return (
    <RadioGroupContext.Provider value={{ value, forwardRef }}>
      <div class="ant-radio-group ant-radio-group-outline">{props.children}</div>
...
const RadioButton: Component<{
  ...
  return (
    <label
      ...
      classList={{ "ant-radio-wrapper-checked": value?.() === props.value }}
    >
      <span
        class="ant-radio"
        classList={{ "ant-radio-checked": value?.() === props.value }}
      >
        <input
          ref={forwardRef}
          type="radio"
